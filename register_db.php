...
if (isset($_POST['m_name'])) {
    ...
    $m_level    = $_POST['m_level'] ?? 'member';
    $m_name     = $_POST['m_name'] ?? '';
    $m_email    = $_POST['m_email'] ?? '';
    $m_tel      = $_POST['m_phone'] ?? ''; // ฟอร์มยังใช้ m_phone แต่ DB ใช้ m_tel
    $m_address  = $_POST['m_address'] ?? '';
    $m_status   = $_POST['m_status'] ?? 1;
    ...
    if ($row_count > 0) {
        ...
        if ($upload != '') {
            $typefile = strtolower(strrchr($_FILES['m_img']['name'], "."));
            ...
            if ($typefile == '.jpg' || $typefile == '.jpeg' || $typefile == '.png') {
                ...
                if (move_uploaded_file($_FILES['m_img']['tmp_name'], $path_copy)) {
                    ...
                    $stmt_insert = $conn->prepare("
                        ...
                        (m_username, m_password, m_level, m_name, m_email, m_tel, m_address, m_status, m_img)
                        VALUES
                        (:m_username, :m_password, :m_level, :m_name, :m_email, :m_tel, :m_address, :m_status, :m_img)
                    ");
                    ...
                    $stmt_insert->bindParam(':m_img', $newname, PDO::PARAM_STR);
                ...
                echo '<script>
                    setTimeout(function() {
                        swal({
                            ...
                            text: "อนุญาตเฉพาะไฟล์ .jpg, .jpeg, .png",
                            type: "error"
                        }, function() {
                            window.location = "register.php";
                        });
