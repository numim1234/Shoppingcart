 <?php
  ...
  if (isset($_POST['m_username']) && isset($_POST['m_password'])) {
    ...
    //รับพารามิเตอร์ redirect เพื่อส่งกลับหน้าที่ผู้ใช้ต้องการ
    $redirect = '';
    ...
    if ($stmt->rowCount() == 1) { //เช็คสถานะว่าเป็นแอดมิน
      ...
      if ($_SESSION['m_level'] == 'admin') { //เช็คสถานะว่าเป็นแอดมิน
        //ถ้ามี redirect ให้ไปหน้าที่ขอมา (ตรวจสอบความปลอดภัยแบบง่าย)
        if (!empty($redirect) && stripos($redirect, 'http') === false && strpos($redirect, '//') === false) {
        ...
        if (!empty($redirect) && stripos($redirect, 'http') === false && strpos($redirect, '//') === false) {
          ...
          header("Location: member/index.php"); //login ถูกต้องและกระโดดไปหน้าตามที่ต้องการ
        }
      }
